...
export class ReunionService {
  ...
  crear_reunion(reunion: ReunionDto): any {
    return this.http.post<any>(`https://siget-equipo2.herokuapp.com/reuniones/create?temas=${reunion.temas}&descripcion=${reunion.descripcion}&horaInicio=${reunion.horaInicio}&horaFin=${reunion.horaFin}&asistentes=${reunion.asistentes}&convocante=${reunion.convocante}`, {});
  }
  postId;
  errorMessage;
  ...
  getByAsistentes(name: string): Observable<ReunionDto[]> {
    return this.http.get<any>(`https://siget-equipo2.herokuapp.com/reuniones/get?asistentes=${name}`)
    .pipe(
